...
function firstRun(){
	...
	for(let i = 1; i <= lastMonth; i++){
		let month = document.createElement("option"); // 12월까지 추가하고.
		month.value= i;
		month.innerHTML = i;
...
function setMaxDay(month){
	...
	let previousDay = select_day.value; // 사용자가 기존에 날짜를 선택했을 경우를 대비해 날짜 저장.
	...
	for(let i = 1; i <= maxDay[month-1]; i++){
		let day = document.createElement("option"); // Day 추가.
		day.value= i;
		day.innerHTML = i;
	...
	if(previousDay != ""){
		if(previousDay <= maxDay[month-1]){
			select_day.value = previousDay; // 사용자가 선택한 이전 day로 복구.  
		}else{
			select_day.value = maxDay[month-1]; // 예) 이전 day가 31일인데, 1월일 경우 30일로 설정.
		}
	} 
}
...
function removeDay(){
	...
	while(select_day.firstChild){
		select_day.removeChild(select_day.firstChild); // 첫번째 child가 없다고 할때까지 첫번째 child 제거.
	}
}
...
function validateId(){
	...
	let result = regExp.test(input_id.value);
...
function validatePwd(){
	...
	if(input_id.value == input_pwd.value){ // 아이디, 비밀번호는 달라야한다.
	...
	let regExp = /^[A-Za-z0-9]{4,12}$/; // 4~12자의 영문, 숫자인가?
	let result = regExp.test(input_pwd.value);
	...
	let input_pwdConfirm = document.getElementById("input_pwdConfirm");
	
	if(input_pwd.value != input_pwdConfirm.value){ // 비밀번호 확인란은 비밀번호랑 같아야한다.
		window.alert("비밀번호 확인: 비밀번호 확인란을 비밀번호란과 똑같이 입력해야합니다");
		input_pwdConfirm.value = "";
...
function validateMail(){
	...
	let result = regExp.test(input_mail.value);
...
function validateName(){
	...
	let result = regExp.test(input_name.value);
	if(!(result)){
...
function validateJoomin(){
	let input_joomin = document.getElementById("input_joomin");
	let regExp = /^\d{13}$/; // 주민번호는 13자리의 숫자다.
	let joomin = input_joomin.value;
